...
import { Table, Download, Eye, ChevronDown, ChevronUp } from 'lucide-react';

export function LedgerTable() {
  const { plan, results } = usePlanner();
  const { annualTimeline, kpis } = results;
  ...
  return (
    <section className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden transition">
      ...
      <div className="p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            onClick={exportToCSV}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-xl transition"
      ...
      >
        <table className="w-full text-left text-xs">
          <thead className="bg-slate-50 dark:bg-slate-800/80 text-slate-600 dark:text-slate-300 sticky top-0 font-semibold border-b border-slate-200 dark:border-slate-800">
            <tr>
              <th className="py-2.5 px-4">Age (Year)</th>
              <th className="py-2.5 px-3">Phase</th>
              <th className="py-2.5 px-3 text-right">Start Portfolio</th>
              <th className="py-2.5 px-3 text-right">Incomes / Savings</th>
              <th className="py-2.5 px-3 text-right">Returns</th>
              <th className="py-2.5 px-3 text-right">Spend / Drawdown</th>
              <th className="py-2.5 px-3 text-right">Taxes Paid</th>
              <th className="py-2.5 px-4 text-right">End Portfolio</th>
            </tr>
          </thead>
      ...
      <div className="p-3 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800 text-center">
        <button
          onClick={() => setIsExpanded(!isExpanded)}
          className="text-xs font-semibold text-brand-600 dark:text-brand-400 hover:underline flex items-center justify-center gap-1 mx-auto"
        >
          {isExpanded ? (
            <>
